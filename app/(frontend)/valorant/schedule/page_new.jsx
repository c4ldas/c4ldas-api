...
export default function ValorantSchedule({ params, searchParams }) {
  ...
  async function showFormatted(event) {
    ...
    const request = await fetch("/api/valorant/schedule/vlr.gg?" +
      new URLSearchParams({
        channel: "$(channel)",
        type: 'text',
        id: id,
  ...
  async function generateCode(event) {
    ...
    const responseCode = `.me $(touser) ► $\{customapi.${origin}/api/valorant/schedule/vlr.gg?channel=$(channel)&id=${id}&msg="${msg}"\}`;
  ...
  return (
    <div className="container">
      ...
      <main className="main block">
        ...
        <div style={{ paddingTop: "10px" }}><code onClick={copyToClipboard} id="code" className="code">$(touser) ► $(customapi.{origin}/api/valorant/schedule/vlr.gg?channel=$(channel)&id=<span className="red">LEAGUE_ID</span>)</code></div>
        ...
        <code onClick={copyToClipboard} id="code" className="code">$(touser) ► $(customapi.{origin}/api/valorant/schedule/vlr.gg?channel=$(channel)&id=<span className="red">2449</span>)</code>
