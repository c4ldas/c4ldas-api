...
export default function TFT({ params, searchParams }) {
  ...
  async function showFormatted(e) {
    ...
    const request = await fetch("/api/tft/rank?" +
      new URLSearchParams({
        channel: "$(channel)",
        type: 'json',
        msg: msg,
  ...
  async function generateCode(e) {
    ...
    const responseCode = `.me $(sender) ► $\{customapi.${origin}/api/tft/rank?channel=$(channel)&type=text&${values}&msg="${msg}"\}`;
  ...
  return (
    <div className="container">
      ...
      <main className="main block">
        <h1>TFT - Teamfight Tactics Rank</h1>
        <h3>How to use this endpoint on Streamelements</h3>
        <div style={{ paddingTop: "10px" }}><code className="code">$(touser) ► $(customapi.{origin}/api/tft/rank?channel=$(channel)&region=<span className="red">REGION</span>&player=<span className="red">PLAYERNAME</span>&tag=<span className="red">TAG</span>&type=text)</code></div>
