...
function listTodayMatches(data) {
  leagueName = data.data.matchSeries.items[0].tournament.name;
  ...
  for (let i = 0; i < matchList.length; i++) {
    ...
    if (matchDate.getDate() == today.getDate()) {
      const game = { startTime: match.startTime, team1Name: match.contestants[0]?.team.name, team2Name: match.contestants[1]?.team.name, team1Score: match.contestants[0]?.score, team2Score: match.contestants[1]?.score }
...
// Placeholder function, need to be implemented
async function sendResponse(data) {
  ...
  const games = matches.map((match) => {
    // Get hours and append an 'h' if needed
    const hour = match.startTime.split("T")[1].split(":")[0] - 3;
    return `${hour}h - ${match.team1Name} ${match.team1Score}x${match.team2Score} ${match.team2Name}`
  });

  const message = msg.replaceAll(/\(league\)/g, leagueName);

  if (type != "text") {
    console.log(games.join(" // "));
    console.log(leagueName);
    return NextResponse.json({ matches, title: leagueName }, { status: 200 });
  }
