...
export async function GET(request, { params }) {
  try {
    ...
    if (!question) return NextResponse.json({ status: "failed", error: "Question missing" }, { status: 400 });
    if (!options || options.length < 2 || options.length > 10) return NextResponse.json({ status: "failed", error: "Invalid number of options. Min: 2, Max: 10" }, { status: 400 });
